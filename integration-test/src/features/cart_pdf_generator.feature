Feature: Cart receipt generation triggered by a Queue message containing BizEvent data
    ...
    Scenario: Successful PDF generation and status update for an INSERTED cart receipt
        Given a cart with id "cart-receipt-success-1" and cartId "tr-success-1" and status "INSERTED" stored into cart datastore
    ...
    Scenario: Successful PDF generation and status update for a RETRY cart receipt
        Given a cart with id "cart-receipt-success-2" and cartId "tr-success-2" and status "RETRY" stored into cart datastore
    ...
    Scenario: Cart receipt is discarded if status is not INSERTED or RETRY
        Given a cart with id "cart-receipt-success-3" and cartId "tr-success-3" and status "FAILED" stored into cart datastore
    ...
    Scenario: a biz event stored on cart-receipts-message-error is enqueued on receipt queue that trigger the PDF receipt generation
        Given a cart with id "cart-receipt-retry-1" and cartId "tr-retry-1" and status "RETRY" stored into cart datastore
